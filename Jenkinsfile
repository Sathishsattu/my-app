node{
   stage('SCM Checkout'){
     git 'https://github.com/Sathishsattu/my-app'
   }
   ...
   sh 'docker build -t sathishsattu/myweb:0.0.2 .'
   }
   stage('Docker Image Push'){
   withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
   sh "docker login -u sathishsattu -p ${dockerPassword}"
    }
   ...
   sh 'docker run -d -p 8090:8080 --name tomcattest sathishsattu/myweb:0.0.2' 
   }
}
